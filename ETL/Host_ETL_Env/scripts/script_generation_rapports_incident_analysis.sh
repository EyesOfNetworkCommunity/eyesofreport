...
do 
	...
	if [ $nb_record -eq 1 ]; then
		echo $(date +"%Y-%m-%d %r") "L'application $application est bien liee au contexte de contrat EFS_24_7_Haute_Dispo" >> $month_folder/creation_rapports.log
		contract_context_id_custom=$contract_context_id
		contract_context_name_custom="Full"
	else 
		echo $(date +"%Y-%m-%d %r") "L'application $application n'est pas liee au contexte de contrat EFS_24_7_Haute_Dispo" >> $month_folder/creation_rapports.log
		...
		if [ -z "$contract_context_id_custom" ]; then
			continue=0
			echo $(date +"%Y-%m-%d %r") "L'application $application n'est liee a aucun contexte de contrat" >> $month_folder/creation_rapports.log
		else 
