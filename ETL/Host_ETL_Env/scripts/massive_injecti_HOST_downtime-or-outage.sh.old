...
if [ ! "`head -1 ${DataDir}/${FILEIN}`" == "Valid;Date debut;Heure debut;Date fin;Heure fin;Type;Host" ]; then
		echo ""
		echo ""
...
for FileLine in `cat ${DataDir}/${FILEIN} | grep -v "Valid;Date debut;Heure debut;Date fin;Heure fin;Type;Host" | sed -e 's: :%:g' | sed -e 's:\[:\\\[:g' | sed -e 's:\]:\\\]:g'` ; do
	...
	for Equipment in `echo $FileLine | cut -d';' -f7 | sed -e 's/:/ /g'`; do
		if [ "$TYPE" = "DOWNTIME" ]; then
			echo "Adding Downtime for $Equipment from ${SYEAR}/${SMONTH}/${SDAY} ${SHOUR}:${SMINUTE} to ${EYEAR}/${EMONTH}/${EDAY} ${EHOUR}:${EMINUTE}"
			${SetDowntimeScript} ${USER} ${PASSWD} Host ${BACKEND} ${SYEAR}_${SMONTH}_${SDAY}_${SHOUR}:${SMINUTE} ${EYEAR}_${EMONTH}_${EDAY}_${EHOUR}:${EMINUTE} $Equipment
		fi
		if [ "$TYPE" = "OUTAGE" ]; then
			echo "Adding Outage for $Equipment from ${SYEAR}/${SMONTH}/${SDAY} ${SHOUR}:${SMINUTE} to ${EYEAR}/${EMONTH}/${EDAY} ${EHOUR}:${EMINUTE}"
			${SetOutageScript} ${USER} ${PASSWD} Host ${BACKEND} ${SYEAR}_${SMONTH}_${SDAY}_${SHOUR}:${SMINUTE} ${EYEAR}_${EMONTH}_${EDAY}_${EHOUR}:${EMINUTE} $Equipment
		fi
	done
done
