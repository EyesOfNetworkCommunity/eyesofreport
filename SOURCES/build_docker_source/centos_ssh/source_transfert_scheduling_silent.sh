...
if [ -d "$source_path" ]; then
	...
	if [ $configure_nagios_source -eq 1 ]; then
		...
		nagios_continue=1
		while [ -n "$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_nagios_log.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_nagios_log.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')
			sed -i -e "$myLine"'d' /etc/cron.d/eyesofreport
		done

		if [ $nagios_continue -eq 1 ]; then
			cron_line="15 2 * * * $cron_command >> $source_path/log_transfert_nagios_log"
			echo "$cron_line" >> /etc/cron.d/eyesofreport
			...
			echo -e "Transfert scheduling for log nagios ($nagios_log_path) in /etc/cron.d/eyesofreport	 \e[92m[OK] \e[39m"
		fi
	...
	if [ $configure_nagios_bp_source -eq 1 ]; then
		...
		nagiosbp_continue=1
		while [ -n "$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_nagios_bp.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_nagios_bp.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')
			sed -i -e "$myLine"'d' /etc/cron.d/eyesofreport
		done
		...
		if [ $nagiosbp_continue -eq 1 ]; then
			...
			chown -R eyesofreport:wheel $source_path
			echo -e "Transfert scheduling for nagios bp ($nagios_bp_log_path) in /etc/cron.d/eyesofreport	 \e[92m[OK] \e[39m"
		fi
	...
	if [ $configure_ged_source -eq 1 ]; then
		...
		ged_continue=1
		while [ -n "$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_ged.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_ged.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')
			sed -i -e "$myLine"'d' /etc/cron.d/eyesofreport
		done
		...
		if [ $ged_continue -eq 1 ]; then
			...
			chown -R eyesofreport:wheel $source_path
			echo -e "Transfert scheduling for ged ($nagios_ged_path) in /etc/cron.d/eyesofreport	 \e[92m[OK] \e[39m"
		fi
	...
	if [ $configure_lilac_source -eq 1 ]; then
		...
		lilac_continue=1
		while [ -n "$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_lilac.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/cron.d/eyesofreport | grep -n -m 1 "transfert_lilac.sh $eor_address $eor_ssh_port" | awk -F ":" '{print $1}')
			sed -i -e "$myLine"'d' /etc/cron.d/eyesofreport
		done
		...
		if [ $lilac_continue -eq 1 ]; then
			...
			chown -R eyesofreport:wheel $source_path
			echo -e "Transfert scheduling for lilac ($nagios_lilac_path) in /etc/cron.d/eyesofreport	 \e[92m[OK] \e[39m"
		fi
