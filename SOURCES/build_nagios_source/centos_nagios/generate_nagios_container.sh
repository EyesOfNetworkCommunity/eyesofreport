...
cp $installation_path/Host_Files.tar /srv/eyesofreport
...
if [ "$drop_database" == "y" ]; then 
	...
	MYSQL_PWD="$MYSQL_ROOT" mysql -uroot < $installation_path/generic_nagiosbp.sql
...
if [ "$drop_database" == "y" ]; then 
	...
	MYSQL_PWD="$MYSQL_ROOT" mysql -uroot < $installation_path/generic_lilac.sql
...
if [ "$drop_database" == "y" ]; then 
	...
	MYSQL_PWD="$MYSQL_ROOT" mysql -uroot < $installation_path/generic_ged.sql
...
pentaho_continue=1
		while [ -n "$(cat /etc/crontab | grep -n -m 1 "docker exec -it -d $container_name /srv/eyesofreport/scripts/insert_nagios_to_thrukDB.sh" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/crontab | grep -n -m 1 "docker exec -d $container_name /srv/eyesofreport/scripts/insert_nagios_to_thrukDB.sh" | awk -F ":" '{print $1}')
			while true; do
				read -p "Nagios log insert job for $site_source is already scheduled in /etc/crontab on eyes of report server. Do you want to remove it ? (y/n)" yn
				case $yn in
		...
		if [ $pentaho_continue -eq 1 ]; then
			echo "30 2 * * * root docker exec -it -d $container_name /srv/eyesofreport/scripts/insert_nagios_to_thrukDB.sh" >> /etc/crontab
			chmod +x /srv/eyesofreport/scripts/insert_nagios_to_thrukDB.sh
			echo -e "Nagios log insert job for $site_source	 \e[92m[OK] \e[39m"
		fi

pentaho_continue=1
		while [ -n "$(cat /etc/crontab | grep -n -m 1 "docker exec -it -d $container_name /srv/eyesofreport/scripts/insert_eon_DB.sh $site_source" | awk -F ":" '{print $1}')" ]; 
		do
			myLine=$(cat /etc/crontab | grep -n -m 1 "docker exec -d $container_name /srv/eyesofreport/scripts/insert_eon_DB.sh $site_source" | awk -F ":" '{print $1}')
			while true; do
				read -p "Livestatus sql dump insert job for $site_source is already scheduled in /etc/crontab on eyes of report server. Do you want to remove it ? (y/n)" yn
				case $yn in
					[Yy]* ) sed -i -e "$myLine"'d' /etc/crontab;break;;
		...
		if [ $pentaho_continue -eq 1 ]; then
			echo "40 2 * * * root docker exec -it -d $container_name /srv/eyesofreport/scripts/insert_eon_DB.sh $site_source" >> /etc/crontab
