...
<div id="page-wrapper">
    <div class="row">
        <h1 class="page-header">Category Customer Access</h1>
    </div>
    ...
    <div class="panel panel-default">
        <div style="overflow-y: auto; overflow-x: hidden; height: 800px;">
            <div class="row">
                            ...
                            <div id=\"".$bp_source_uniq."\" >
                                <div style=\"overflow-y: auto; overflow-x: hidden; height: 200px;\">                          
                                    <ul class=\"source connected\" id=\"ul_".$bp_source_uniq."\">";
                                        ...
                                        AND (bp_link like '%_ca' OR bp_link like'%_CA' )
                                        AND bp_link NOT IN (SELECT bp_name FROM bp_category WHERE category = 'Customer Access' 
                ...
                <div class="col-xs-6">
                    <div class="col-md-12">
                        <h4>Business process set to Category Customer Access</h4>
                    </div>
                    <div class="col-md-12">
                        <ul class="target connected">
                            <?php
                            $sql = "SELECT * FROM bp_category WHERE category = 'Customer Access' ORDER BY bp_source";
                            if(!$result = $db->query($sql)){
                                die('There was an error running the query [' . $db->error . ']');
                            }
                            while($row = $result->fetch_assoc()){ 
                                echo "<li>".$row['bp_source']."-->".$row['bp_name']."</li>";
                            } 
                            $result->free();
                            ?>
                        </ul>
