...
<?php
	...
	$backend_selected=mysql_result($sqlresult,0,"auth_type");
	if($backend_selected=="1"){
		$ldap_ip=mysql_result($sqlresult,0,"ldap_ip");
                $ldap_port=mysql_result($sqlresult,0,"ldap_port");
                $ldap_search=mysql_result($sqlresult,0,"ldap_search");
                $ldap_filter=mysql_result($sqlresult,0,"ldap_filter");
                $ldap_user=mysql_result($sqlresult,0,"ldap_user");
                $ldap_password=mysql_result($sqlresult,0,"ldap_password");
        	$ldap_rdn=mysql_result($sqlresult,0,"ldap_rdn");
	}	
	...
	// If Update button pressed
		...
		// Retrieve authentification backend settings
	        ...
	        $backend_selected=mysql_result($sqlresult1,0,"auth_type");
	        ...
	        elseif($backend_selected=="1"){
	                $ldap_ip=mysql_result($sqlresult1,0,"ldap_ip");
	                $ldap_port=mysql_result($sqlresult1,0,"ldap_port");
	                $ldap_search=mysql_result($sqlresult1,0,"ldap_search");
	                $ldap_filter=mysql_result($sqlresult1,0,"ldap_filter");
	                $ldap_user=mysql_result($sqlresult1,0,"ldap_user");
	                $ldap_password=mysql_result($sqlresult1,0,"ldap_password");
	                $ldap_rdn=mysql_result($sqlresult1,0,"ldap_rdn");
			...
			ldap_set_option($ldapconn, LDAP_OPT_REFERRALS, 0);
				...
				$db = mysql_select_db($database_eonweb, $connexion);
				if (!$db){
					echo "<ul>";
					echo "<li class='msg_title'>Alert EyesOfNetwork - Message EON-database </li>";
					echo "<li class='msg'> Could not open database : $database_eonweb ($database_host)</li>";
					echo "</ul>";
					exit(1);
				}
				...
				foreach (array('a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','1','2','3','4','5','6','7','8','9','0') as $c){
					...
					message(6,"Listing ".$ldap_current_filter."* users ","ok");
					...
					$sr=ldap_search($ldapconn, $ldap_search, $ldap_current_filter, array("dn" ,"$ldap_rdn","name"));
						...
						for($i=0;$i<$info["count"];$i++){
							...
							$resq=mysql_query("UPDATE ldap_users_extended SET dn='".$dn."', login='".$info[$i][$ldap_rdn]["0"]."', user='".$username."', checked=1 where dn='".$dn."'",$connexion);
							//$resq=mysql_fetch_array($resq);
							if($resq[0]==0){
                                               			mysql_query("INSERT INTO ldap_users_extended VALUES('".$dn."','".$info[$i][$ldap_rdn]["0"]."','".$username."',1)",$connexion);
				...
				mysql_query("DELETE FROM ldap_users_extended WHERE checked=0",$connexion);
